...
for i = 1:num_states
    ...
    data_all_subjects.(current_state) = zeros(2001, num_subjects);
end

% Loop through each subject and store the data
for sub = 1:num_subjects
    for i = 1:num_states
        current_state = microstates(i);
        if numel(current_stim_site) > 1
            data_all_subjects.(current_state)(:, sub) = ...
                data_struct(sub).(current_stim_site(1)).occurrences_clr_bc.(current_state) ...
                - data_struct(sub).(current_stim_site(2)).occurrences_clr_bc.(current_state);
        else
            data_all_subjects.(current_state)(:, sub) = ...
                data_struct(sub).(current_stim_site).occurrences_clr_bc.(current_state);
            data_microstate(sub, :) = data_struct(sub).(current_stim_site).occurrences_clr_bc.(current_state)';
        end
    end
end
...
for i = 1:num_states
    ...
    average_state = mean(data_microstate, 1);
    average_matrix(i, :) = average_state;
end

% Calculate the average and standard deviation for each field
averages = struct();
std_devs = struct();
for i = 1:num_states
    current_state = microstates(i);
    averages.(current_state) = mean(data_all_subjects.(current_state), 2); % Average over subjects
    std_devs.(current_state) = std(data_all_subjects.(current_state), 0, 2); % Standard deviation over subjects
end
...
for i = 1:num_states
    current_state = microstates(i);
    standard_errors.(current_state) = std_devs.(current_state) / sqrt(num_subjects);
end

% Calculate the critical t-value for 95% confidence interval
alpha = 0.05;
degrees_of_freedom = num_subjects - 1;
...
for i = 1:num_states
    ...
    if ~isempty(pre_indices)
        x_pre = [common_time_array(time_indices(pre_indices)), fliplr(common_time_array(time_indices(pre_indices)))];
        y_pre = [upper(pre_indices), fliplr(lower(pre_indices))];
    ...
    if ~isempty(post_indices)
        x_post = [common_time_array(time_indices(post_indices)), fliplr(common_time_array(time_indices(post_indices)))];
        y_post = [upper(post_indices), fliplr(lower(post_indices))];
